 ...
 */
let Quiz = function(quizContainer, answers) {
...
Quiz.prototype.checkAnswers = function(flagUnanswered) {
    ...
    for (let i = 0; i < this.questions.length; i++) {
        ...
        // Get answers
        let answerInputs = question.getElementsByClassName(Quiz.Classes.QUESTION_ANSWERS)[0].getElementsByTagName('input');
        let input = null;
...
Quiz.prototype.clearHighlights = function(question) {
    ...
    let questionWarnings = question.getElementsByClassName(Quiz.Classes.QUESTION_WARNING);
    while (questionWarnings.length > 0) {
        questionWarnings[0].parentNode.removeChild(questionWarnings[0]);
    }

    // Remove highlighted elements
    let highlightedQuestions = [question.getElementsByClassName(Quiz.Classes.CORRECT), question.getElementsByClassName(this.Classes.INCORRECT)];
    ...
    let tempElements = question.getElementsByClassName(Quiz.Classes.TEMP);
